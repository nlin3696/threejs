...
// Font matcap
const fontMatcapTexture = textureLoader.load('/textures/matcaps/3.png')
fontMatcapTexture.colorSpace = THREE.SRGBColorSpace
...
houseG.add(bush1, bush2, bush3, bush4)
...
const signFace = new THREE.Mesh(
    new THREE.BoxGeometry(1.7, 0.8, 0.05, 1, 1, 1),
    signMaterial,
)
signFace.position.y += 0.75
signG.position.z += 2.2
signG.position.x += -1.6
signG.add(signFace)


const signFolder = gui.addFolder('Sign')
signFolder.add( signG.position, 'z', -10, 10, 0.01)
signFolder.add( signG.position, 'x', -10, 10, 0.01)
signFolder.add( signFace.rotation, 'y', -10, 10, 0.1).name('Sign Face')
...
floorFolder.add( floor.position, 'y', -1, 2)
floorFolder.add( floor.rotation, 'x', -5, 5)
...
houseG.add(doorLight)

const signLight = new THREE.PointLight('#ff7d46', 2)
signLight.position.set(0, 2.2, 2.5)
signLight.position.x = -1.5
signLight.position.y = 1
houseG.add(signLight)


gui.add(signLight.position, 'y', 0, 5, 0.001).name('Sign Light pos y')
gui.add(signLight.position, 'x', -4, 5, 0.001).name('Sign Light pos x')
scene.add(floor)
...
fontLoader.load(
    ...
    (font) => {
        ...
        // textGeometry.computeBoundingBox()
        textGeometry.center()
        const material = new THREE.MeshMatcapMaterial({ matcap: fontMatcapTexture })
        const text = new THREE.Mesh(textGeometry, material)
        text.position.y = 0.75
        gui.add(text.position, 'y', 0, 5, 0.001).name('Sign Light pos y')
    gui.add(text.position, 'x', -4, 5, 0.001).name('Sign Light pos x')
    signG.add(text)
...
for (const grave of (gravesG.children)) {
